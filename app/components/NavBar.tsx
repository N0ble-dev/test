...
const NavBar = () => {
  ...
  return (
    <header
      ...
      } fixed inset-0  max-h-[5rem] lg:max-h-[8rem]  z-[99]    bg-[#F5F5F5] flex flex-col gap-2  py-4 transition-all duration-300 ${
        isScrollingDown
          ? "translate-y-[-101%]"
          : !isTopPage && !isScrollingDown
          ? `  -translate-y-8 lg:-translate-y-10 ${!isHome && "bg-white/80"}`
          : "translate-y-0"
      }`}
    >
      <nav className=" flex flex-col">
        ...
        <div className=" -mt-4 lg:-mt-2 pt-4 w-full bg-white">
